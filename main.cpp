...
//  实际并没有扣除这个消耗，只是测试出来，作为一个参考, 说明实际的性能还要高一些
...
void   test_malloc_range_and_memset( const unsigned int uint_malloc_size_in, const unsigned int uint_test_count_in, const bool bool_show_in )
{
    std::vector<void*>  vt_ptr_void;
    vt_ptr_void.resize( uint_test_count_in, nullptr );
    ...
    const unsigned int  uint_malloc_size_mask = uint_malloc_size_in - 1;
    void*  ptr_void_tmp;
    unsigned int  uint_malloc_size;
    for( unsigned int i=0; i<uint_test_count_in; ++i )
    {
        uint_malloc_size = ( i & uint_malloc_size_mask ) + 1;
        ptr_void_tmp = malloc( uint_malloc_size );
        vt_ptr_void[i] = ptr_void_tmp;
        memset( ptr_void_tmp, 0, uint_malloc_size );
        continue;
    }
    const haisql::timet_nanoseconds  timet_nanoseconds_malloc_end = haisql::now_steady_nanoseconds();
    for( unsigned int i=0; i<uint_test_count_in; ++i )
    ...
    if( bool_show_in )
    {
        ...
        const unsigned long long  ulong_malloc_use_nansecond = timet_nanoseconds_malloc_end - timet_nanoseconds_begin;
        const unsigned long long  ulong_malloc_free_use_nansecond = timet_nanoseconds_free_end - timet_nanoseconds_begin;
        std::cout << "malloc+memset() 1-" << uint_malloc_size_in << "Byte use="  << ulong_malloc_use_nansecond << ", avg=" << ulong_malloc_use_nansecond / uint_test_count_in
                  <<  std::endl;
        std::cout << "malloc+memset+free() 1-" << uint_malloc_size_in << "Byte use=" << ulong_malloc_free_use_nansecond << ", avg=" << ulong_malloc_free_use_nansecond /uint_test_count_in
                  << std::endl;
    }
    return;
}
...
void  test_run( const unsigned int  uint_test_count_in, const bool bool_show_in ) noexcept
{
    ...
    //test_malloc_range_and_memset( 256, uint_test_count_in, bool_show_in );

     //  测试 1---1024字节
    test_malloc_range( 1024, uint_test_count_in, bool_show_in );
    test_malloc_real_random( 1024, uint_test_count_in, bool_show_in );
    ...
    /*{
        const unsigned int  uint_test64k_count = uint_test_count_in / 12800;
        for( unsigned int j=1; j<=8; ++j )
        {
            test_malloc( j*128*4096, uint_test64k_count, bool_show_in );
            continue;
        }
    }*/
    return;
...
void  test_loop( const unsigned int  uint_test_count_in, const unsigned int uint_loop_count_in, const bool bool_show_in, const unsigned int uint_thread_number_in ) noexcept
{
    ...
    for( unsigned int i=0; i<uint_loop_count_in; ++i )
    {
        if( bool_show_in )
        {
            std::cout << std::endl << "memory loop i=" << i << ", test_count=" << uint_test_count_in << std::endl;
        }
...
//  下面这行代码实现只需要C++11即可， 不需要支持C++14，实现了编译器gcc4.8.5下可以编译
#if __cplusplus >= 201103L
#if __cplusplus < 201401L
namespace  std
{
template <typename T, typename... Types>
inline  unique_ptr<T>  make_unique( Types &&... args ) noexcept
{
    T *t_ptr = new  T( std::forward<Types>( args )... );
    return  unique_ptr<T>( t_ptr );
}
} // namespace  std
#endif // __cplusplus
#endif // __cplusplus
